...
} else if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_cart'])) {
    $sc_id = $_POST['sc_id'];

    // Example: delete the item from cart using sc_id
...
<body>
    ...
    if ($user_id > 0) {
        ...
        while ($cartItem = $cartResult->fetch_assoc()) {
            ...
            // Get product details FIRST (always needed for display)
            ...
            if ($productResult->num_rows > 0) {
                ...
                if ($stockResult->num_rows > 0) {
                    ...
                    if ($stock['on_sale_quantity'] < $quantity) {
                        // Insufficient stock - but we have product details
                        $cartItems[] = [
                            'available' => false,
                            'sc_id' => $sc_id,
                            'product_id' => $product_id,
                            'on_sale_quantity' => $stock['on_sale_quantity'],
                            'message' => 'Only ' . $stock['on_sale_quantity'] . ' available',
                ...
                // If we reach here, product is available
                ...
                $cartItems[] = [
                    ...
                    'name' => 'Deleted Product #' . $product_id,
                    'image_path' => '',
                    'price' => 0
                ];
            }
            ...
            $productQuery->close();
            if (isset($stockQuery)) $stockQuery->close();
        }
                                        ...
                                        <?php foreach ($cartItems as $item): ?>
                                            <tr>
                                                ...
                                                <td class="cart_product_remove">
                                                    <input type="hidden" name="sc_id" value="<?= htmlspecialchars($item['sc_id'] ?? '') ?>">

                                                    <button type="submit" name="delete_cart" class="fa fa-trash-o">
                                                        Delete Cart
                                                    </button>
                                                </td>
